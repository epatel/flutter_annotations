...
class CodeBuilder {
  ...
  void generateFiles(String sourceDir) {
    ...
    if (_generatedCode.isEmpty) {
      ...
      print('✅ Generated ${_generatedCode.length} code sections');
    }
  ...
  String _buildEmptyOutputFile() {
    ...
    buffer.writeln('// Generated by builder.dart at ${DateTime.now()}');
    buffer.writeln();
    buffer.writeln('// No annotations found to generate extensions for');
    buffer.writeln();
  ...
  String generateImportPath(String filePath) {
    ...
    if (normalizedFilePath.startsWith(normalizedSourceDir)) {
      final relativePath = path.relative(normalizedFilePath, from: normalizedSourceDir);
...
class AnnotationVisitor extends RecursiveAstVisitor<void> {
  ...
  void visitClassDeclaration(ClassDeclaration node) {
    ...
    for (final metadata in node.metadata) {
      ...
      if (processor != null) {
        final generatedCode = processor.processAnnotation(node, className, filePath);
